 ...
 *
 * Controller:
 *   ./sched_demo <num_hogs> <runtime_seconds>
 *
 * Required behavior:
 *   1) fork+exec CPU hogs FIRST
 *   2) fork+exec interactive LAST (passing runtime_seconds)
 *   3) waitpid() for interactive
 *   4) kill hogs
 *   5) reap remaining children
 *
 * Build:
 *   gcc -O2 -Wall -Wextra -std=c11 sched_demo.c -o sched_demo
 */
